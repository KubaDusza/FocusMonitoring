...
def setup():
    ...
    if "recorded_directions" not in st.session_state:
        st.session_state.recorded_directions = []  # Store 4 recorded directions as rotation vectors

    if "zone_threshold" not in st.session_state:
        st.session_state.zone_threshold = np.radians(30)  # Allow 30 degrees error
...
def plot_3d_pose(rotation_vector, plot_placeholder, colored_face_index=1):
    ...
    x_axis_correction = R.from_euler('x', -90, degrees=True).as_matrix()
    y_axis_correction = R.from_euler('y', 180, degrees=True).as_matrix()
    ...
    if colored_face_index == 1:
        face_center = np.mean(faces[1], axis=0)
