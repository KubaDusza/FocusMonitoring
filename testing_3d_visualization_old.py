...
def setup():
    ...
    if "axis_corr" not in st.session_state:
        st.session_state.axis_corr = {"x": 0, "y": 0, "z": 0}

    if "rotation_corr" not in st.session_state:
        #st.session_state.axis_corr = {"x": -90, "y": 0, "z": 180}
        st.session_state.rotation_corr = [0,0,0]
...
def calibrate_axis_corrections(rotation_vector):
    ...
    """
    st.session_state.rotation_corr = rotation_vector

    st.success("Calibration complete! Current pose set as neutral.")


#rotation_buffer = deque(maxlen=2)
...
def plot_3d_pose(rotation_vector, plot_placeholder, colored_face_index=0, x_axis_corr = -90, y_axis_corr = 0, z_axis_corr = 180):
    ...
    """
    #global rotation_buffer, translation_buffer

    # Add current vectors to buffer for smoothing
    #rotation_buffer.append(rotation_vector)

    # Compute moving average for smoothing
    #rotation_vector = np.median(rotation_buffer, axis=0)

    # Convert rotation vector to rotation matrix
    #print(rotation_vector)
    rotation_vector -= st.session_state.rotation_corr

    rotation = R.from_rotvec(rotation_vector[:3])
    rotation_matrix = rotation.as_matrix()


    #Apply a -90 degree rotation around the X-axis for coordinate correction
    ...
    # Colors for the faces: cyan for default, red for the colored face
    face_colors = ['cyan'] * 6
    face_colors[colored_face_index] = 'red'  # Color the specified face differently

    # Add the faces to the plot with specified colors
    for idx, face in enumerate(faces):
        ax.add_collection3d(
            Poly3DCollection([face], facecolors=face_colors[idx], linewidths=1, edgecolors='k', alpha=.75))

    # If the colored face is the back (index 1), draw a smiley face on it
    if colored_face_index == 0:
        face_center = np.mean(faces[0], axis=0)

        # Define smiley face components (relative to the back face vertices)
        # Eyes
        ...
        # Mouth (as a simple arc defined by a few points)
...
def main():
    ...
    # Setup placeholders for displaying frames and plots
    placeholder = st.empty()
    data_placeholder = st.empty()

    #st.session_state.axis_corr["x"] = st.slider(label="x axis correction", max_value=180, min_value=-180, value=-90)
    #st.session_state.axis_corr["y"] = st.slider(label="y axis correction", max_value=180, min_value=-180, value= 0)
    #st.session_state.axis_corr["z"] = st.slider(label="z axis correction", max_value=180, min_value=-180, value= 180)
    ...
    if enable_camera:
        ...
        while enable_camera:
            ...
            try:
                ...
                #data_placeholder.markdown(f"Rotation Vector: {head_pose_data['rotation_vector']}\n"
                #                          f"Translation Vector: {head_pose_data['translation_vector']}")
                ...
                if show_3d_visualization and head_pose_data["rotation_vector"] is not None:
                    plot_3d_pose(
                        ...
                        #head_pose_data["translation_vector"],
                        plot_placeholder,
